...
describe('update', () => {
    skipIfWindows('tests the updater', async () => {
        await qq.rm([
            ...
            '.local',
            'share',
            'oclif-example-s3-cli',
        ])
        await qq.x(
            'aws s3 rm --recursive s3://oclif-staging/s3-update-example-cli'
        )
        ...
        pjson.name = `s3-update-example-cli-${Math.floor(
            Math.random() * 100000
        )}`
        ...
        const checkVersion = async (
            version: string,
            nodeVersion = pjson.oclif.update.node.version
        ) => {
            const stdout = await qq.x.stdout(
                `./tmp/${pjson.oclif.bin}/bin/${pjson.oclif.bin}`,
                ['version']
            )
            expect(stdout).to.equal(
                `${pjson.oclif.bin}/${version} ${process.platform}-${process.arch} node-v${nodeVersion}`
            )
        }
