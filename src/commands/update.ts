    ...
    async run() {
        ...
        if (flags['from-local']) {
            ...
            const versions = fs
                ...
                .filter(
                    (dirOrFile) =>
                        dirOrFile !== 'bin' && dirOrFile !== 'current'
                )
            ...
            const pinToVersion = await cli.prompt(
                'Enter a version to update to'
            )
            ...
            if (
                !(await fs.pathExists(path.join(this.clientRoot, pinToVersion)))
            ) {
                throw new Error(
    ...
    protected async fetchManifest(): Promise<IManifest> {
        ...
        if (!this.config.scopedEnvVarTrue('USE_LEGACY_UPDATE')) {
            try {
                ...
                const { body } = await http.get<IManifest | string>(
                    newManifestUrl
                )
...
`
            ...
            await fs.symlink(
                `./${version}`,
                path.join(this.clientRoot, 'current')
            )
        }
    }
